...
(async () => {
    ...
    try {
        ...
        const css = '/* --- Mirii Sewing Brand Guidelines --- */' +
            ':root {' +
            ...
            '.sns-share, .sns-follow, .author-info, .copy-info, .toc, .post-meta, .footer-meta, .footer-bottom-logo, .sns-copy-message, .tagcloud, .widget_recent_entries, .widget_recent_comments, .widget_archive, .widget_categories { display: none !important; }' +
            '/* Full Width Layout for Major Pages */' +
            '.page-id-18 #main, .page-id-24 #main, .page-id-25 #main { width: 100% !important; max-width: 100% !important; padding: 0 !important; margin: 0 !important; }' +
            '.page-id-18 .container, .page-id-24 .container, .page-id-25 .container { max-width: none !important; width: 100% !important; padding: 0 !important; }' +
            '.page-id-18 #content, .page-id-24 #content, .page-id-25 #content { margin-top: 0 !important; }' +
            '/* Premium UI Elements */' +
        ...
        const html_1day = '<!-- wp:cover {"url":"' + images.fv_1day + '","dimRatio":20,"overlayColor":"black","align":"full"} -->' +
            ...
            '    <h4 style="margin-top:35px; font-size:1.6rem;">A. 裏地付きワンピース</h4>' +
            '    <p>初心者でも綺麗に仕上がる基礎を学びます。そのままお出かけに着ていける上品なデザインです。</p>' +
            '  </div>' +
            '  <div style="text-align:left;">' +
            '    <img src="' + images.work_b + '" class="brand-img" style="aspect-ratio:1/1;"/>' +
            '    <h4 style="margin-top:35px; font-size:1.6rem;">B. 男の子用パンツ</h4>' +
        ...
        const html_welcome = '<!-- wp:cover {"url":"' + images.fv_welcome + '","dimRatio":20,"overlayColor":"black","align":"full"} -->' +
            ...
            '      <h3 style="border-left:6px solid var(--mirii-pink-dark); padding-left:30px; margin-bottom:30px; font-size:1.8rem;">1. 「完成すること」より「習得すること」</h3>' +
            '      <p style="font-size:1.15rem;">理屈が分かれば、教室を出た後もお一人で応用ができるようになります。なぜそうするのか、という「裏側」を丁寧にお伝えします。</p>' +
            '    </div>' +
            '    ' +
            '    <div>' +
            '      <h3 style="border-left:6px solid var(--mirii-pink-dark); padding-left:30px; margin-bottom:30px; font-size:1.8rem;">2. 「依存」ではなく「自立」へ</h3>' +
        ...
        console.log("Pages complete. Verifying...");
