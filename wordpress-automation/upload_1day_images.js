...
(async () => {
    ...
    // 存在するファイル名に修正
    const images = [
        { name: 'IMG_8857.JPG', path: '/Users/ayumi/Desktop/HP素材/お洋服/IMG_8857.JPG' },
        { name: 'IMG_6860.JPG', path: '/Users/ayumi/Desktop/HP素材/お洋服/IMG_6860.JPG' },
        { name: 'IMG_9758.JPG', path: '/Users/ayumi/Desktop/HP素材/お洋服/IMG_9758.JPG' },
        { name: 'IMG_6045.JPG', path: '/Users/ayumi/Desktop/HP素材/お洋服/IMG_6045.JPG' }
    ];
    ...
    let oldResults = {};
    if (fs.existsSync('uploaded_1day_images.json')) {
        try {
            oldResults = JSON.parse(fs.readFileSync('uploaded_1day_images.json'));
        } catch (e) { }
    }
    const finalResults = { ...oldResults, ...results };
    fs.writeFileSync('uploaded_1day_images.json', JSON.stringify(finalResults, null, 2));
    console.log("Done. Results in uploaded_1day_images.json");
    await browser.close();
})();
